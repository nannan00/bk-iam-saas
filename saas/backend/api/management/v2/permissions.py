...
from backend.api.management.models import ManagementObjectAPIAllowListConfig
...
class ManagementAPIPermission(permissions.IsAuthenticated, ManagementAPIPermissionCheckMixin):
    """
    管理类 API 权限校验，校验的参数主要来自 request.data、request.query_params、url_kwargs 参数和 request 认证过的数据
    需要配合 management_api_permission
    management_api_permission = {
        "请求函数名称": (鉴权所需参数来源，鉴权 API 名称)
    }
    鉴权所需参数来源对应是：VerifyAPIParamLocationEnum 枚举值
    鉴权 API 名称对应是：ManagementAPIEnum 枚举值
    对于需要忽略鉴权的请求函数，可配置 IGNORE_VERIFY_API_CONFIG，即{"请求函数名称": IGNORE_VERIFY_API_CONFIG}
    """
    ...
    def has_permission(self, request, view):
        ...
        # 若配置了当前鉴权 Class，但是没有配套的 management_api_permission，则直接不通过
        if not hasattr(view, "management_api_permission"):
            return False

        # 默认值即为请求 method，因为继承于 APIView（即使是 ViewSet，ViewSet 也是继承了 APIView 的）
        handler_name = request.method.lower()
        # ViewSet.as_view({...}) 设置了 method 的映射，则进行更新为映射的值
        if hasattr(view, "action"):
            handler_name = view.action

        # 判断要处理的函数，即 handler 是否配置了权限，
        # 若没有配置权限控制，则直接不通过，若实际需要忽略鉴权的，可显示配置 IGNORE_VERIFY_API_CONFIG
        ...
        # 若校验不通过，则会直接抛出 exceptions.PermissionDenied 异常
    ...
    def _verify_api(self, view, request, param_source, api):
        """
        根据 param_source 获取 request.data、request.query_params、url_kwargs 中的数据，对 API 进行校验
        """
        # 所有 API 认证鉴权所需
        ...
        # 对于涉及特定对象的操作，先检查对象级白名单旁路
        # 若白名单命中则直接放行，跳过后续的系统级和对象级校验
        if param_source != VerifyApiParamLocationEnum.SYSTEM_IN_PATH.value:
            if self._is_object_allow_list_matched(view, request, param_source, api, app_code):
                return
        ...
        if param_source in [
            ...
            # 由于 URL 参数是必然会获取得到的，否则是已经 404 的，不会执行到这里，所以可以直接 [] 获取而不需要判断是否存在
    ...
    def _is_object_allow_list_matched(self, view, request, param_source, api, app_code):
        """检查对象级白名单是否命中，命中返回 True 表示可以跳过系统级鉴权"""
        # 单个对象：从 URL 取对象 ID
        if param_source in [
            VerifyApiParamLocationEnum.ROLE_IN_PATH.value,
            VerifyApiParamLocationEnum.GROUP_IN_PATH.value,
    ...
    def _try_extract_group_ids(self, request, param_source):
        """尝试从请求中提取 group_ids 用于白名单校验，提取失败返回 None"""
        try:
            if param_source == VerifyApiParamLocationEnum.GROUP_IDS_IN_BODY.value:
                slz = ManagementGroupIDsSLZ(data=request.data)
                if slz.is_valid():
                    return slz.validated_data["group_ids"]
            elif param_source == VerifyApiParamLocationEnum.GROUP_IDS_IN_QUERY.value:
                group_ids_str = request.query_params.get("group_ids")
                if group_ids_str:
                    return list(map(int, group_ids_str.split(",")))
            elif param_source == VerifyApiParamLocationEnum.GROUPS_IN_BODY.value:
                slz = ManagementGroupsSLZ(data=request.data)
                if slz.is_valid():
                    return [g["id"] for g in slz.validated_data["groups"]]
    ...
    def _verify_api_by_role(self, app_code: str, role_id: int, api: ManagementAPIEnum, system_id: str):
        """
        由于管理员 API 有很大一部分都是在需要在分级管理员角色下操作的
        所以该方法主要是封装了对角色下的 API 校验
        """
        # 对象级白名单旁路：命中则直接放行，不走系统级鉴权
        if ManagementObjectAPIAllowListConfig.is_allowed(app_code, VerifyAPIObjectTypeEnum.ROLE.value, role_id, api):
            return

        # 查询分级管理员的来源系统
        role_source = RoleSource.objects.filter(source_type=RoleSourceType.API.value, role_id=role_id).first()
        # 角色来源不存在，说明页面创建或默认初始化的，非 API 创建，所以任何系统都无法操作
        if role_source is None:
            # 如果 role 是对应 system_id 的系统管理员，则其来源则默认为传入的系统
            if (
                system_id
    ...
    def _verify_api_by_group(self, app_code: str, group_id: int, api: ManagementAPIEnum, system_id: str):
        """
        对用户组下的对象进行操作时，比如增删成员、添加权限等，需要进行 API 认证鉴权
        所以该方法主要是封装了对用户组下的 API 校验
        """
        # 对象级白名单旁路：命中则直接放行，不走系统级鉴权
        if ManagementObjectAPIAllowListConfig.is_allowed(app_code, VerifyAPIObjectTypeEnum.GROUP.value, group_id, api):
            return
    ...
    def _verify_api_by_object(
        ...
        其实上是对 verify_api_by_role 和 verify_api_by_group 的统一封装
        """
    ...
    def _verify_api_by_template(self, app_code: str, template_id: int, api: ManagementAPIEnum, system_id: str):
        """
        对权限模板下的操作进行 API 认证鉴权
        通过 RoleRelatedObject 找到模板所属角色，再调用 _verify_api_by_role 完成校验
        """
        # 对象级白名单旁路：命中则直接放行，不走系统级鉴权
        if ManagementObjectAPIAllowListConfig.is_allowed(
            app_code, VerifyAPIObjectTypeEnum.TEMPLATE.value, template_id, api
        ):
            return
    ...
    def _verify_api_by_groups(self, app_code: str, group_ids: List[int], api: ManagementAPIEnum, system_id: str):
        """
        批量操作用户组，需要进行 API 认证鉴权
        所以该方法主要是封装了对用户组下的 API 校验
        """
        # 对象级白名单旁路：所有用户组都在白名单中则直接放行
        if all(
