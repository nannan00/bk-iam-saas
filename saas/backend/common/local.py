...
class Local(Singleton):
    ...
    @property
    def request_tenant_id(self) -> str:
        try:
            # celery 后台，openAPI 都可能没有 user，需要判断
            if self.request and (self.request.META.get("HTTP_X_BK_TENANT_ID") or hasattr(self.request, "user")):
                return (
                    self.request.META.get("HTTP_X_BK_TENANT_ID") or self.request.user.get_property("tenant_id") or ""
                )
        except Exception:  # pylint: disable=broad-except
            return ""
