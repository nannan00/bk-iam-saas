...
### 功能优化
* 管理类 API 白名单校验支持向上级联：用户组、权限模板可继承其所属分级管理员的白名单配置
* 优化离职交接记录创建逻辑，统一收敛到交接业务层处理
...
### 新增功能
* 权限交接支持接入 ITSM 审批流程，审批单据详情支持审批类型展示及表格分页
* 超管类接口新增用户组自定义权限的查询和更新接口
* 管理类 API 支持对象级（object-level）白名单配置，并将 bk_aidev 系统加入管理类 API 白名单
* 权限续期时长上限延长至两年

### 缺陷修复
* 修复权限模板可勾选超过 10 个的问题
* 修复 mgmt_api_update_templates 更新接口因缺失参数导致调用失败的问题
* 修复管理空间用户组添加 admin 用户静默成功的问题
* 修复 ES2020 可选链导致编译失败的问题
* 修复审批工单路由跳转路径错误的问题
...
### 缺陷修复
* 修复人员离职转岗交接和回收接口失败时仅返回 FAILED，改为返回实际失败原因
...
### 缺陷修复
* 修复单租户分支全新部署时 migration 迁移文件报错的问题
* 修复用户组添加权限前资源名称检查的资源 ID 未去重问题
...
### 缺陷修复
* 修复权限到期清理功能失效的问题
* 修复操作审批流程更新接口跨系统篡改的安全风险
* 锁定 axios 版本避免自动升级依赖导致的问题
* 修复用户组权限申请默认页面样式
...
### 缺陷修复
* 修复资源实例表格操作按钮组溢出未展示完整
* 修复分级管理员成员中不存在的用户导致校验报错，改为自动跳过
* 修复同步新用户时对后台待删除用户重复创建导致报错的问题
...
### 缺陷修复
* 修复 ANY_ID（任意）实例校验展示名称导致多语言场景下校验失败的问题
* 修复首次设置系统管理员拥有该系统所有权限时不生效
* 修复人员离职转岗处理接口 URL 错误及 ancestor_department_ids 属性调用错误
* 修复自定义权限申请和临时权限申请时永久有效期被过期时间校验拒绝的问题
* 修复分级管理员审批通过后同步子集管理员授权范围任务在事务提交前执行的问题
...
### 缺陷修复
* 修复成员校验 validate_members_compat_string_list 问题
...
### 新增功能
* 个人信息栏新增个人中心的跳转链接
* 管理员自动拥有所有操作权限入口基于环境变量开启
* 人员选择器支持组织架构外显
* 提供离职员工权限的查询、清理、交接接口
* 人员离职转岗时支持清理 IAM 的分级管理员数据
* 超管类 API 新增组织架构同步接口
* 网关新增 admin_list_groups 和 admin_list_subject_groups 接口

### 功能优化
* 权限中心 V3 增加环境变量控制权限中心 V4 入口的展示
* 组织架构外显优化，仅搜索出的人员才外显组织架构
* 成员存在多个组织架构时支持数量 tag 展示
* 最长已过期权限删除期限修改为 3 个月

### 缺陷修复
* 修复超长权限模板名称显示错误
* 修复申请自定义权限时权限获得者处理逻辑异常
* 修复 V4 访问地址错误
* 修复修改管理空间实例后管理员用户组未能保持一致
* 修复修改管理员接口对于传入数据校验不严格的问题
* 修复 admin API 网关权限问题
* 修复有效期突破前端限制的问题
* 修复用户组权限模板删除缺少权限校验
...
### 功能优化
* 组织架构选择器支持精确匹配的时候才默认勾选
* 申请自定义权限支持仅当权限获得者只是当前登录用户本身时才进行已有个人权限回显
* 申请加入用户组支持仅当权限获得者只是当前登录用户本身时才进行已有用户组回显

### 缺陷修复
* 修复组织架构选择器切换到组织架构选项会默认勾选
* 修复组织架构选择器切换到人员模板选项会默认勾选
...
* 修复交接历史页面点击切换导航栏失效
* 修复偶发交接失败问题
* 修复用户组组权限个别自定义权限实例 path 数量不同步
* 修复 policy 中 instance path 过长时造成的审计日志过大问题
...
### 新增功能
* 支持通过 API 方式创建权限模板
* 权限中心 V3 提供权限中心 V4 的切换入口
* 用户组列表支持不可被申请字段筛选
* 定期补偿单据状态未更新的周期任务添加时间过滤条件
* 请求用户管理接口 page_size 支持通过环境变量控制

### 功能优化
* 优化审计日志，截断策略实例以防止日志过大

### 缺陷修复
* 修复用户组续期邮件提醒未生效问题
* 修复同步时用户/部门数据未分批入库的问题
* 修复分级管理员接口注册地址错误
* 修复接口性能问题
...
### 缺陷修复
* 修复申请自定义权限、临时权限申请的申请期限自定义期限无法回显
* 修复授权白名单表的唯一约束错误问题
...
* 分页参数由 limit/offset 调整为 page/page_size
* 性能优化 - 每月审计表全局变量缓存
* Swagger 重构
...
### 功能优化
* OpenAPI 分页参数调整为 page_size/page，对于 admin.list_group/admin.list_group_member/mgmt.list_group/mgmt.list_group_member 已开放接口，兼容 limit/offset
...
### 功能优化
* apigw 新增 user-groups/department-groups 两个开放 API
...
### 缺陷修复
* 修复前端用户组配置权限相关问题
* 组织架构部门同步修复了部门 lft/rght/level 数据错误问题
...
### 缺陷修复
* 修复更新模板未同步到用户组权限问题
* 修复非 80/443 端口时 cookie domain 错误问题
* 取消勾选对应权限的实例报错
...
### 修复
* [ 修复 ] 我的权限 - 加入用户组时间错误
* [ 修复 ] 申请或配置权限页面 - 时间选择自定义时 icon 与文字重叠问题
